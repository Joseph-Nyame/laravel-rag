...
class IngestFileJob implements ShouldQueue
{
    ...
    public function handle(
        ...
        try {
            ...
            $filteredPayload = array_diff_key($pointPayload, array_flip([
                'agent_id',
                'file_type',
                'original_filename'
            ]));
    ...
    protected function generateSchemaFromPoint(array $pointPayload, OpenAI\Client $openAI): array
    {
        try {
            $prompt = "Analyze the following data point from a CSV to define a schema:\n" .
                      ...
                      "- 'optional': Array of field names that are optional (empty, null, or likely missing in other records).\n" .
                      ...
                      "  - 'manual': Boolean, set to true for fields requiring system generation (e.g., 'uuid', 'incremental', 'timestamp', 'agent_id').\n" .
                      "Infer types based on patterns:\n" .
                      "- 'agent_id' or agent identifier → 'number', manual: true.\n" .
                      ...
                      "Do not assume specific formats (e.g., 'pin', 'phone_number', 'enum:male,female') unless explicitly clear from the data. " .
            ...
            if (!is_array($schema) || empty($schema)) {
                ...
                foreach ($pointPayload as $field => $value) {
                    ...
                    $specifics[$field] = [
                        ...
                        'example' => $value ?? '',
                        'generator_type' => $type,
                        'manual' => in_array($field, ['agent_id', 'id', 'identifier', 'created_at', 'updated_at']),
                    ];
                }
                return [
                    'required' => array_filter(array_keys($pointPayload), fn($key) => !empty($pointPayload[$key])),
                    'optional' => array_filter(array_keys($pointPayload), fn($key) => empty($pointPayload[$key])),
                    'specifics' => $specifics,
                ];
            }

            // Ensure manual flags for specific fields
            foreach ($schema['specifics'] as $field => &$spec) {
                if (in_array($field, ['agent_id', 'id', 'identifier', 'created_at', 'updated_at']) || in_array($spec['type'], ['uuid', 'incremental', 'timestamp'])) {
                    $spec['type'] = match ($field) {
                        'agent_id' => 'number',
                        'id' => 'incremental',
                        'identifier' => 'uuid',
                        'created_at', 'updated_at' => 'timestamp',
